...
/obj/machinery/atmospherics/pipe/mains/multiz/DirectConnection(datum/pipeline/querying, obj/machinery/atmospherics/source)
	...
	var/obj/machinery/atmospherics/mains/multiz/above = locate(/obj/machinery/atmospherics/pipe/simple/multiz) in(SSmapping.get_turf_above(T))
	var/obj/machinery/atmospherics/mains/multiz/below = locate(/obj/machinery/atmospherics/pipe/simple/multiz) in(SSmapping.get_turf_below(T))
	if(above)
		. |= above
	if(below)
		. |= below
