...
/obj/machinery/rnd/production/proc/user_try_print_id(id, amount)
	...
	if(isnull(amount))
		amount = 1
	var/datum/design/D = (linked_console || requires_console)? (linked_console.stored_research.researched_designs[id]? SSresearch.design_by_id(id) : null) : SSresearch.design_by_id(id)
	if(!istype(D))
		return FALSE
